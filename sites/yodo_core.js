...
const ORDER_CART_URLS = [
  ...
  async function getOrderCartNext(page) {
    // Your real DOM often is: <a id="sc_i_buy" class="yBtnText"><span>次へ進む</span></a>
    return await pickVisible(
      page.locator('#sc_i_buy'),
      page.locator('a#sc_i_buy'),
      page.locator('.yBtnStack .yBtnText'),
      // text variants that appear on order cart
  ...
  async function addToCartAndCheckout(page, { url, quantity = 1, autoClick = true, cvv }) {
    ...
    // 6) Optional CVV
    if (cvv) {
      const cvvInput = page.locator('input[type="password"], input[name*="security"], input[name*="cvv"]').first();
      if (await cvvInput.count()) { await cvvInput.fill(String(cvv)).catch(()=>{}); await page.waitForTimeout(80); }
    }
  
    // 7) Tick "セキュリティコードを入力しない" if present
    await skipSecurityCodeIfAvailable(page);
